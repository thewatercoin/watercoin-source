...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "watercoind"]) is not None:
            print("%sWARNING!%s There is already a watercoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "WATERCOIND" not in os.environ:
        os.environ["WATERCOIND"] = build_dir + '/src/watercoind' + exeext
        os.environ["WATERCOINCLI"] = build_dir + '/src/watercoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `watercoin-cli help` (`rpc_interface.txt`).
